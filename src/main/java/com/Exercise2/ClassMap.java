...
public class ClassMap {
	...
	private final int NUMBER_OF_ROW = 3;
	private final int NUMBER_OF_COL = 3;
	...
	public boolean addRow(boolean existing, String strMapline) { //validate at exercise2 if line is correct format
		...
		if(existing) {
			...
			while(!size3) {
				for(int i = 0; i < 3; i++) {
					key = Validation.enterNumofChars("\nEnter key for column " + (i+1) + " : ", DEFAULT_LENGTH_OF_VALUES);
					value = Validation.enterNumofChars("\nEnter value for column " + (i+1) + " : ", DEFAULT_LENGTH_OF_VALUES);
	...
	public void edit() {
		...
		while(!existing) {
			...
			if(iRowIndex <= mapArray.size() && iColIndex <= NUMBER_OF_COL
		...
		//get new values
		strNewkey = Validation.enterNumofChars("Enter new key : ", DEFAULT_LENGTH_OF_VALUES);
		strNewValue = Validation.enterNumofChars("Enter new value : ", DEFAULT_LENGTH_OF_VALUES);
